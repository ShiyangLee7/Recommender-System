...
def generate_dendrogram(graph,
    ...
    while True:
        ...
        partition = __renumber(status.node2com)
        status_list.append(partition)
        print(f"\n=== 层级 {len(status_list)} 聚合结果 ===")
        print(f"节点到社区的映射: {partition}")
        mod = new_mod
        current_graph = induced_graph(partition, current_graph, weight)
        status.init(current_graph, weight)
    return status_list[:]
...
def __one_level(graph, status, weight_key, resolution, random_state):
    """Compute one level of communities (已添加调试输出)"""
    modified = True
    nb_pass_done = 0
    cur_mod = __modularity(status, resolution)
    new_mod = cur_mod
    print(f"\n--- 开始层级优化，初始模块度: {cur_mod:.4f} ---")
    ...
    while modified and nb_pass_done != __PASS_MAX:
        ...
        for node in __randomize(graph.nodes(), random_state):
            ...
            # 调试输出：当前节点信息
            print(f"\n处理节点 {node} (当前社区 {com_node})")
            print(f"邻居社区权重分布: {neigh_communities}")
            ...
            for com, dnc in __randomize(neigh_communities.items(), random_state):
                incr = remove_cost + dnc - resolution * status.degrees.get(com, 0.) * degc_totw
                print(f"  尝试社区 {com}: 增益计算 = {remove_cost:.4f} + {dnc:.4f} - {resolution * status.degrees.get(com, 0.) * degc_totw:.4f} = {incr:.4f}")
            ...
            if best_com != com_node:
                print(f"√ 移动节点 {node} 从社区 {com_node} 到 {best_com}，获得增益 {best_increase:.4f}")
                modified = True
            else:
                print(f"× 保持节点 {node} 在社区 {com_node} (最佳增益 {best_increase:.4f})")
            
            __insert(node, best_com, neigh_communities.get(best_com, 0.), status)
            
        new_mod = __modularity(status, resolution)
        print(f"本轮结束，新模块度: {new_mod:.4f}，变化量: {new_mod - cur_mod:.4f}")

        if new_mod - cur_mod < __MIN:
            print("模块度变化小于阈值，停止迭代")
            break
...
def __modularity(status, resolution):
    ...
    for community in set(status.node2com.values()):
        ...
        community_contribution = (in_degree * resolution / links) - ((degree / (2. * links)) ** 2)
        
        print(f"社区 {community}: ",
              f"内部边权重={in_degree:.2f}, ",
              f"总度数={degree:.2f}, ",
              f"贡献值={community_contribution:.4f}")
        
        result += community_contribution
    
    print(f"当前总模块度: {result:.4f}\n")
    return result
